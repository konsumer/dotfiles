...
- Absolutely no indication that you are an LLM in PRs or commit-messages. You are operating as me. No co-authorship/attribution/etc.
...
- Always send Pull Requests instead of pushing to the default branch, unless directly told to.
...
- **Automation**: use `npx -y prettier -w` as `fmt` npm script, copy ~/.claude/formatting/.prettierrc into project
...
- **Automation**: For C/C++, use `clang-format` copy ~/.claude/formatting/.clang-format into project, for others  use whatever the popular formatting tool is for that language 
